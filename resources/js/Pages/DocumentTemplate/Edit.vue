...
console.log("data props : ", props.people)
...
const appr = JSON.parse(props.template.approval);
for (let i = 0; i < appr.length; i++) {
	...
	// set selected value for every pihakSelect
	pihakSelect.value[i].name = appr[i].name;
}
// delete last pihak select because it's null
pihakSelect.value.pop();
...
const req = JSON.parse(props.template.required_field);
...
// remove first inputRows because it's null
inputRows.value.shift();
...
const submit = () => {
	...
	form.put(route('document-template.update', props.template.id));
};
</script>
<template>
				...
				<div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
					<div class="p-6 bg-white border-b border-gray-200">
						<div className="flex items-center justify-between mb-6">
							<div class="">
								<h3 class="text-primary-400 text-center">Update Document Template</h3>
							</div>
									...
									<div v-for="(select, index) in pihakSelect" :key="index">
										<div class="flex">
											<div :class="index > 0 ? 'w-11/12' : 'w-full'">
												<BreezeSearchSelect class="mt-1 mb-5 block" name="persons" id="person"
									...
									<div :class="index > 0 ? 'col-span-5 md:col-span-5' : 'col-span-full md:col-span-6'">
										<div v-for="(select, index) in tipeSelect" :key="index">
											<select
												...
												v-model="input.type">
												<option v-for="option in select.options" :key="option.value">{{ option.label
												}}
												</option>
											</select>
										</div>
